...
import { fetchDeviceDiagnosticData } from "../../../../services/api";
...
function Diagnostics() {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetchDeviceDiagnosticData(
        deviceId,
        startISO,
        endISO,
        token
      ...
      if (!csvLines) {
        toast.error("No diagnostics available for the selected date range.");
        return;
      }

      const zip = new JSZip();
      zip.file("system_diagnostics.csv", csvLines);

      const content = await zip.generateAsync({ type: "blob" });
      saveAs(content, "system_diagnostics.zip");
